...
def estimate_regions(mlevel, t, pixels, width, height):
    ...
    for i in range(width*height):
        val = 0xff & pixels[i]
        pixel_position = (i % width, i / width)
        for k in range(mlevel):
            if k < mlevel-1:
                if t[k] < val < t[k + 1]:
                    rep[k].putpixel(pixel_position, val)
            else:
                if val > t[k]:
                    rep[k].putpixel(pixel_position, val)
    return rep
...
def save_regions(regions,levels,origin_file):
    ...
    for i in range(levels):
        file_name = os.path.join(head, 'reg_%d%s' % (i, tail))
...
def get_grayscaled(file_name):
    ...
    file_name = os.path.join(head, 'grayscale_' + tail)
...
def plot_histogarm(image,thresholds,file_name):
    head, tail = os.path.split(file_name)
    file_name = os.path.join(head, 'histogram_' + tail)
